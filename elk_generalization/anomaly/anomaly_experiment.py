...
def main(args):
    train_path = os.path.join(
        args.experiments_dir, f"{args.model}/AE/test/AE_lr_log_odds.pt"
    )
    train_logodds = get_logodds(train_path)

    # probe trained on AE and evaluated on AH test
    eval_normal_path = os.path.join(
        args.experiments_dir, f"{args.model}/AH/test/AE_lr_log_odds.pt"
    )
    ...
    eval_anomaly_path = os.path.join(
        args.experiments_dir, f"{args.model}/BH/test/AE_lr_log_odds.pt"
    )
    ...
    print(
        f"AUROC for {args.model} using {args.method}: "
    ...
    out_path = f"{args.out_dir}/{args.method}_{model_last}.json"
